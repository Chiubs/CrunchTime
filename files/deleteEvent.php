...
<?php
  ...
  if (isset($_POST["eventName"]))
  {
          if ($_POST["eventName"])
          {
            ...
            if ($valid){
                  $sql = "DELETE FROM events WHERE ('$event', '$end', '$userid','$eventName')";
                  $results = mysqli_query($conn, $sql);
                  if ($results) {
                    $message =  "Event deleted: $eventName [$date at $time]";
...
<br>
      ...
      <form action = "deleteEvent.php" method="post">
        ...
        <button type = "submit" name = "submit" id = "submit">Delete Event</button>
      </form>
      <?php
            echo $message;
       ?>
